...
contract Stakable is Context, ERC20 {
    
    using SafeMath for uint;
    mapping(address => bool) internal _allowedTokens;
    mapping(address => Strategy) internal _tokenStrategies;
    mapping(address => mapping(address => uint256)) internal _deposits;    
    // mapping(address => mapping(address => uint256)) internal _equity;
    mapping(address => uint) internal rates;
    mapping(address => uint) _depositTotals;
    mapping(address => address[]) _depositers;
    enum Actions{ MINT, DEDUCT_MINT }
    enum Actors{ POOL, TREASURY }
    ...
    constructor(address payable treasury) public {
        ...
        strategy.Distributions.push(Distribution(Actors.TREASURY, 10));
    }
    ...
    function addStakableToken(address _token) public onlyOwner notLocked {
        _allowedTokens[_token] = true;
    }

    function removeStakableToken(address _token) public onlyOwner notLocked {
        _allowedTokens[_token] = false;
    }

    function stakedBalance(address _token) public view returns(uint) {
        return _deposits[_token][_msgSender()];
    }

    function getTotalStaked(address _token) public view returns (uint) {
        return _depositTotals[_token];
    }

    function stake(address _token, uint256 _amount) public returns (bool){
...
contract ConfigurableERC20 is Stakable {
    ...
    constructor(address payable treasury) Stakable(treasury) public {
    ...
    function mint(address account, uint256 amount) public canMint notLocked {
        // uint amount = amount.mul(uint256(10) ** 18);
